...
def draw_mot(video_id,gt_path):
    ...
    file_path_img = 'data/gmot/val/' + video_id + '/img1'  # img图片路径
    # 生成新的文件夹来存储画了bbox的图片
    if not os.path.exists('./pic_dets/bee/bee_test/' + video_id):
        os.mkdir('./pic_dets/bee/bee_test/' + video_id)
        print('The ./pic_dets/bee/bee_test/' + video_id + '  have create!')
    save_file_path = './pic_dets/bee/bee_test/' + video_id
...
if __name__ == '__main__':
    gt_dir = "data/gmot/val"
    pred_dir = "results/gmot_det_ori"
    save_dir = "pic_dets/bee/bee_test"
    for name in os.listdir(gt_dir):
        print('The video ' + name.split('.')[0] + ' begin!')
        draw_mot(name.split('.')[0],pred_dir)
