    ...
    for idx in name_dict:
        ...
        for i in range(name_dict[idx]):
            ...
            cv2.rectangle(img, (int(float(box[0])), int(float(box[1]))),
                          ...
                          (0, 255, 0), 2)
            # put_text
            cv2.putText(img, str(int(float(id))) + ' ' + str(int(float(cls))), (int(float(box[0])), int(float(box[1]))),
...
if __name__ == '__main__':
    gt_path = 'results/trackers/BEE-val/bee_test_cascade/data'
    filename = os.listdir(gt_path)
    # print(filename)
    for name in filename:
        if "BEE2212" not in name:
            continue
        print('The video ' + name.split('.')[0] + ' begin!')
        draw_mot(name.split('.')[0],gt_path,save_dir="pic_track/cascade/")
