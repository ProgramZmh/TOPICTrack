...
def draw_mot20(video_id,gt_path, save_dir):
    txt_name = gt_path + '/' + video_id + '.txt' 
    file_path_img = 'data/MOT20/test/' + video_id + '/img1' 
    frame_width = 1920
    frame_height = 1080
    output_video = os.path.join(save_dir,'mot20.mp4')
    ...
    for idx in name_dict:
        ...
        txt = 'MOT20  ' + 'frame: ' + str(frame) + '  num: ' + str(i + 1)
        cv2.putText(img, txt, (0, 35),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.3, (0, 0, 255), 2.6,cv2.LINE_AA)
...
def main():
    ...
    for name in filename:
        ...
        draw_mot20(name.split('.')[0],folder,save_dir="video/")
