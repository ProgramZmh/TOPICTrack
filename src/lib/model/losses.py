...
from src.lib.utils.image import draw_umich_gaussian
...
class DiceLoss(nn.Module):
    ...
    def forward(self, seg_feat, conv_weight, mask, ind, target, batch_num_obj):
        ...
        for i in range(batch_size):
            ...
            y_rel_coord = (y_grid[None,None] - y[i,:num_obj].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).float())/128.#TODO:64
            x_rel_coord = (x_grid[None,None] - x[i,:num_obj].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).float())/128.#TODO:64
