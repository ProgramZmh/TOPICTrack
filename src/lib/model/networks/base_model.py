...
class BaseModel(nn.Module):
    def __init__(self, heads, head_convs, num_stacks, last_channel, opt=None):
        ...
        if opt.trades:
            ...
            self.embed_dim = 128 #TODO:64
            self.maxpool_stride2 = nn.MaxPool2d(2, stride=2)
            self.avgpool_stride4 = nn.AvgPool2d(4, stride=4)
            self.tempature = 5

            self.embedconv = nn.Sequential(
                nn.Conv2d(64, self.embed_dim, kernel_size=1,
                          stride=1, padding=0, bias=True),
                nn.BatchNorm2d(self.embed_dim, momentum=BN_MOMENTUM), nn.ReLU(
                    inplace=True),
            ...
            self.attention_cur = nn.Conv2d(64, 1, kernel_size=(
                opt.deform_kernel_size, opt.deform_kernel_size), stride=1, dilation=(1, 1), padding=(1, 1), bias=True)
            self.attention_prev = nn.Conv2d(64, 1, kernel_size=(
                opt.deform_kernel_size, opt.deform_kernel_size), stride=1, dilation=(1, 1), padding=(1, 1), bias=True)
            self.conv_offset_w = nn.Conv2d(129, opt.deform_kernel_size * opt.deform_kernel_size, kernel_size=(opt.deform_kernel_size, opt.deform_kernel_size),
                                           stride=1, dilation=(1, 1), padding=(1, 1), bias=True) #TODO:64 改为129
            self.conv_offset_h = nn.Conv2d(129, opt.deform_kernel_size * opt.deform_kernel_size, kernel_size=(opt.deform_kernel_size, opt.deform_kernel_size),
                                           stride=1, dilation=(1, 1), padding=(1, 1), bias=True)#TODO:64 改为129
            self.dcn1_1 = DCN_TraDeS(64, 64, kernel_size=(
                3, 3), stride=1, padding=1, deformable_groups=1)
                ...
                if self.opt.trades:
                    ...
                    if not self.opt.inference:
                        ...
                        for temporal_id in range(2, self.opt.clip_len):
                            z['h_volume_prev{}'.format(
                                temporal_id)] = h_volume_aux[temporal_id-2]
                            z['w_volume_prev{}'.format(
                                temporal_id)] = w_volume_aux[temporal_id-2]
                for head in self.heads:
    ...
    def TraDeS(self, cur_feat, pre_img, pre_hm, addtional_pre_imgs, addtional_pre_hms, inference_feats):
        ...
        if self.opt.inference:
            for prev_feat in inference_feats:
                feat_list.append(torch.from_numpy(
                    prev_feat).to(self.opt.device)[:, :, :, :])
    ...
    def CVA_MFW(self, feat_list, support_feats):
        ...
        for idx, feat_prev in enumerate(feat_list[1:]):
            ...
            prop_feat = self.MFW(
                support_feats[idx], tracking_offset, feat_cur, feat_prev, batch_size, h_f, w_f)
        ...
        adaptive_weights = torch.split(
            adaptive_weights, 1, dim=1)  # 3*(B,1,H,W)
    ...
    def MFW(self, support_feat, tracking_offset, feat_cur, feat_prev, batch_size, h_f, w_f):
        ...
        off_deform = self.gamma(tracking_offset, feat_cur,
                                feat_prev, batch_size, h_f, w_f)
    ...
    def gamma(self, tracking_offset, feat_cur, feat_prev, batch_size, h_f, w_f):
        ...
        feat_offs_h = torch.cat(
            (tracking_offset[:, 1:2, :, :], feat_offs), dim=1)
        feat_offs_w = torch.cat(
            (tracking_offset[:, 0:1, :, :], feat_offs), dim=1)

        off_h_deform = self.conv_offset_h(
            feat_offs_h)[:, :, None, :, :]  # (B, 9, H, W)
        off_w_deform = self.conv_offset_w(feat_offs_w)[:, :, None, :, :]
        off_deform = torch.cat(
            (off_h_deform, off_w_deform), dim=2)  # (B, 9, 2, H, W)
