...
def generic_decode(output, K=100, opt=None):
  ...
  if opt.seg:
    ...
    for i in range(batch):
      ...
      y_rel_coord = (y_grid[None, None] - ys[i].unsqueeze(-1).unsqueeze(-1).float()) / 128.#TODO:64
      x_rel_coord = (x_grid[None, None] - xs[i].unsqueeze(-1).unsqueeze(-1).float()) / 128.#TODO:64
