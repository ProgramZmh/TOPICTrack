...
from src.lib.model.model import create_model, load_model
from src.lib.model.decode import generic_decode
from src.lib.model.utils import flip_tensor, flip_lr_off, flip_lr
from src.lib.utils.image import get_affine_transform, affine_transform
from src.lib.utils.image import draw_umich_gaussian, gaussian_radius
from src.lib.utils.post_process import generic_post_process
from src.lib.utils.debugger import Debugger
# from src.lib.utils.tracker import Tracker
from src.deep_sort import tracker
from src.lib.dataset.dataset_factory import get_dataset
from src.lib.model.utils import _nms
from src.lib.model.utils import _tranpose_and_gather_feat
...
class Detector(object):
    ...
    def run(self, opt, image_or_path_or_tensor, meta={}, img_id=None):
        ...
        for scale in self.opt.test_scales:
            ...
            if self.opt.tracking:
                ...
                if self.opt.pre_hm:
                    ...
                    self.pre_hms = self._get_additional_inputs(
                        self.tracks, meta, with_hm=not self.opt.zero_pre_hm)

            pre_process_time = time.time()
            pre_time += pre_process_time - scale_start_time  # 0.009
            ...
            net_time += forward_time - pre_process_time  # 0.076
            decode_time = time.time()
            dec_time += decode_time - forward_time  # 0.012

            # convert the cropped and 4x downsampled output coordinate system
            # back to the input image coordinate system
            result = self.post_process(dets, meta, scale, output)  # 0.001
        ...
        end_time = time.time()
        merge_time += end_time - post_process_time  # 0.0

        if self.opt.tracking:
            public_det = meta['cur_dets'] if self.opt.public_det else None
            # holmescao
            results, self.tracks = tracker.step(results, RESET, opt)
            # Trades
    ...
    def process(self, images, pre_images=None, pre_hms=None, return_time=False):
        with torch.no_grad():
            ...
            if self.opt.trades:
                ...
                while len(self.inference_prehm) < (self.opt.clip_len - 1):
                    ...
                    self.inference_prehm.appendleft(pre_hms)
                output, current_feat = self.model(images, pre_images, pre_hms, inference_feats=self.inference_feats,
                                                  addtional_pre_imgs=self.inference_preim, addtional_pre_hms=self.inference_prehm)
                output = output[-1]
        ...
        torch.cuda.empty_cache()  # 可以减少400 GPUmem
        if return_time:
    ...
    def debug(self, debugger, images, dets, output, scale=1,
        ...
        pred = debugger.gen_colormap(
            output['hm'][0].detach().cpu().numpy(), 'hm')
                ...
                if ('bbox' in item):
                    ...
                    if 'pred_mask' in item:
                        ...
                        debugger.add_coco_seg(
                            mask, item['tracking_id'], img_id='generic', conf=sc)
                # if 'tracking' in item:
