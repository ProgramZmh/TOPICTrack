...
def draw_mot(video_id,gt_path, save_dir):
    ...
    for idx in name_dict:
        ...
        for i in range(name_dict[idx]):
            ...
            if int(float(staff[10].strip())) == 2:
                ...
                cv2.putText(img, str(int(float(id))), (int(float(box[0])), int(float(box[1]))+12),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 2, bottomLeftOrigin=False)
                cv2.putText(img, str(np.round(float(staff[11].strip()),2)), 
                            (int(float(box[0])), int(float(box[1]))-5),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2, bottomLeftOrigin=False)
        ...
        # img = cv2.resize(img,dsize=(10000,6667))
        # 保存图片
...
if __name__ == '__main__':
    gt_path = 'results/trackers/BEE-val/bee_test_ablations_new/data'
    ...
    for name in filename:
        ...
        draw_mot(name.split('.')[0],gt_path,save_dir="pic_track/bee_f_m/")
