...
def draw_mot(video_id,gt_path, save_dir):
    ...
    for idx in name_dict:
        ...
        for i in range(name_dict[idx]):
            ...
            cv2.rectangle(img, (int(float(box[0])), int(float(box[1]))),
                            ((int(float(box[0])) + 20), (int(float(box[1])) + 15)),
            ...
            #             cv2.FONT_HERSHEY_SCRIPT_COMPLEX, 0.6, (0, 0, 255), 2, bottomLeftOrigin=False)
        img = cv2.resize(img,dsize=(1920,1080))
        # img = cv2.resize(img,dsize=(5120,2880))
        # 保存图片
...
if __name__ == '__main__':
    gt_path = '/remote-home/zhengyiyao/topictrack/results/trackers/BEE-val/bytetrack_bee_topic'
    # gt_path = '/remote-home/zhengyiyao/topictrack/results/trackers/BEE-val/trades/data'
    # gt_path = '/remote-home/zhengyiyao/topictrack/results/trackers/BEE-val/ocsort/data'
    filename = os.listdir(gt_path)
    # print(filename)
    for name in filename:
        if "BEE2216" not in name:
            continue
        print('The video ' + name.split('.')[0] + ' begin!')
        draw_mot(name.split('.')[0],gt_path,save_dir="pic_track/bee_fairmot/")
        # draw_mot(name.split('.')[0],gt_path,save_dir="pic_track/bee_trades/")
        # draw_mot(name.split('.')[0],gt_path,save_dir="pic_track/bee_ocsort/")
